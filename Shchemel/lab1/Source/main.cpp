...
bool backtrackingIter(int n, int center, std::vector<std::vector<int>> field, std::vector<Square>& currentStack, int bestStackSize)
{
...
std::vector<Square> findNextFragmentationForDiag(int n, int center, int leftCornerShift)
{
	...
	for (int i = center; i < n; ++i)
	{
		for (int j = center; j < n; ++j)
		{
			bool isEmpty = (i != j || (i != center and j != center)) && (i < leftCornerShift || j < leftCornerShift);
	...
	while (!tmpStack.empty() || retStack.empty())
	{
		...
		{
			bool is_better = backtrackingIter(n, center, field, tmpStack, retStack.size());
